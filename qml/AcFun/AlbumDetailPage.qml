	...
	QtObject {
		...
		function share(){
			if(detail.shareUrl)
            {
                ...
                url += "?url="+encodeURIComponent(detail.shareUrl);
                url += "&type=3";
                url += "&title="+encodeURIComponent(detail.title||"");
                url += "&pic="+encodeURIComponent(detail.cover||"");
